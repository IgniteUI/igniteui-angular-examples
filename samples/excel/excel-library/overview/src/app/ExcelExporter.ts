...
import { FilteringLogic, IFilteringExpression, IFilteringExpressionsTree, IgxBaseExporter, IgxExporterOptionsBase, ISortingExpression, SortingDirection } from "igniteui-angular";
import { getInstanceType } from "igniteui-angular-core";
...
import { ExportUtilities } from "igniteui-angular/esm2015/lib/services/exporter-common/export-utilities";
...
export class WorkbookExportService extends IgxBaseExporter {
    ...
    protected exportDataImplementation(data: any[], options: WorkbookExportOptions) {
        ...
        for (const dataItem of data) {
            ...
            if (isSpecial) {
                this.exportCell(keys[0], excelRow, helper.col, dataItem);
            } else {
                for (const key of keys) {
                    let cellValue = dataItem.rowData[key];
